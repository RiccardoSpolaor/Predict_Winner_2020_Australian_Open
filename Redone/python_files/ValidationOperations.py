    ...
    if best_estimators is None:
        ...
        for n in n_estimators:
            ...
            for i in range(n_tests):
                ...
                # train a decision tree classifier
                estimator = RandomForestClassifier(n_estimators=n, nrandom_state=13, n_jobs=-1)
                estimator.fit(xs, ys)
...
def get_best_dataframe_split(dataframe):
    def get_splitted_dataframe_accuracy(index, dataframe):
        ...
        x_train, x_valid, y_train, y_valid = train_test_split(dataframe.drop(['Winner', 'csvID'], axis=1),
                                                              dataframe['Winner'], test_size=0.33, shuffle=False,
                                                              stratify=None)
        accuracies = []
